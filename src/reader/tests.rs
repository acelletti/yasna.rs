...
fn test_der_read_bool_ok() {
    let tests: &[(bool, &[u8])] = &[(false, &[1, 1, 0]), (true, &[1, 1, 255])];
    for &(evalue, data) in tests {
        let value = parse_der(data, |reader| reader.read_bool()).unwrap();
...
fn test_der_read_bool_err() {
    ...
    for &data in tests {
        parse_der(data, |reader| reader.read_bool()).unwrap_err();
    }
}
...
fn test_ber_read_bool_ok() {
    ...
    for &(evalue, data) in tests {
        let value = parse_ber(data, |reader| reader.read_bool()).unwrap();
        assert_eq!(value, evalue);
    }
}
...
fn test_ber_read_bool_err() {
    ...
    for &data in tests {
        parse_ber(data, |reader| reader.read_bool()).unwrap_err();
    }
}
...
fn test_general_read_i64_ok(mode: BERMode) {
    ...
    for &(evalue, data) in tests {
        let value = parse_ber_general(data, mode, |reader| reader.read_i64()).unwrap();
        assert_eq!(value, evalue);
    }
}
...
fn test_general_read_i64_err(mode: BERMode) {
    ...
    for &data in tests {
        parse_ber_general(data, mode, |reader| reader.read_i64()).unwrap_err();
    }
}
...
fn test_general_read_u64_ok(mode: BERMode) {
    ...
    for &(evalue, data) in tests {
        let value = parse_ber_general(data, mode, |reader| reader.read_u64()).unwrap();
        assert_eq!(value, evalue);
    }
}
...
fn test_general_read_u64_err(mode: BERMode) {
    ...
    for &data in tests {
        parse_ber_general(data, mode, |reader| reader.read_u64()).unwrap_err();
    }
}
...
fn test_general_read_i32_ok(mode: BERMode) {
    ...
    for &(evalue, data) in tests {
        let value = parse_ber_general(data, mode, |reader| reader.read_i32()).unwrap();
        assert_eq!(value, evalue);
    }
}
...
fn test_general_read_i32_err(mode: BERMode) {
    ...
    for &data in tests {
        parse_ber_general(data, mode, |reader| reader.read_i32()).unwrap_err();
    }
}
...
fn test_general_read_u32_ok(mode: BERMode) {
    ...
    for &(evalue, data) in tests {
        let value = parse_ber_general(data, mode, |reader| reader.read_u32()).unwrap();
        assert_eq!(value, evalue);
    }
}
...
fn test_general_read_u32_err(mode: BERMode) {
    ...
    for &data in tests {
        parse_ber_general(data, mode, |reader| reader.read_u32()).unwrap_err();
    }
}
...
fn test_general_read_i16_ok(mode: BERMode) {
    ...
    for &(evalue, data) in tests {
        let value = parse_ber_general(data, mode, |reader| reader.read_i16()).unwrap();
        assert_eq!(value, evalue);
    }
}
...
fn test_general_read_i16_err(mode: BERMode) {
    ...
    for &data in tests {
        parse_ber_general(data, mode, |reader| reader.read_i16()).unwrap_err();
    }
}
...
fn test_general_read_u16_ok(mode: BERMode) {
    ...
    for &(evalue, data) in tests {
        let value = parse_ber_general(data, mode, |reader| reader.read_u16()).unwrap();
        assert_eq!(value, evalue);
    }
}
...
fn test_general_read_u16_err(mode: BERMode) {
    ...
    for &data in tests {
        parse_ber_general(data, mode, |reader| reader.read_u16()).unwrap_err();
    }
}
...
fn test_general_read_i8_ok(mode: BERMode) {
    ...
    for &(evalue, data) in tests {
        let value = parse_ber_general(data, mode, |reader| reader.read_i8()).unwrap();
        assert_eq!(value, evalue);
    }
}
...
fn test_general_read_i8_err(mode: BERMode) {
    ...
    for &data in tests {
        parse_ber_general(data, mode, |reader| reader.read_i8()).unwrap_err();
    }
}
...
fn test_general_read_u8_ok(mode: BERMode) {
    ...
    for &(evalue, data) in tests {
        let value = parse_ber_general(data, mode, |reader| reader.read_u8()).unwrap();
        assert_eq!(value, evalue);
    }
}
...
fn test_general_read_u8_err(mode: BERMode) {
    ...
    for &data in tests {
        parse_ber_general(data, mode, |reader| reader.read_u8()).unwrap_err();
    }
}
...
fn test_general_read_bigint_ok(mode: BERMode) {
    ...
    for &(evalue, data) in tests {
        let value = parse_ber_general(data, mode, |reader| reader.read_bigint()).unwrap();
    ...
    let tests: &[(BigInt, &[u8])] = &[
        (
            ...
            &[
                2, 17, 3, 160, 201, 32, 117, 192, 219, 243, 184, 172, 188, 95, 150, 206, 63, 10,
                210,
            ],
        ),
        (
            BigInt::parse_bytes(b"-1234567890123456789012345678901234567890", 10).unwrap(),
            &[
                2, 17, 252, 95, 54, 223, 138, 63, 36, 12, 71, 83, 67, 160, 105, 49, 192, 245, 46,
            ],
        ),
        (
...
fn test_general_read_bigint_err(mode: BERMode) {
    ...
    for &data in tests {
        parse_ber_general(data, mode, |reader| reader.read_bigint()).unwrap_err();
    }
}
...
fn test_general_read_biguint_ok(mode: BERMode) {
    ...
    for &(evalue, data) in tests {
        let value = parse_ber_general(data, mode, |reader| reader.read_biguint()).unwrap();
...
fn test_general_read_biguint_err(mode: BERMode) {
    ...
    for &data in tests {
        parse_ber_general(data, mode, |reader| reader.read_biguint()).unwrap_err();
    }
}
...
fn test_der_read_bytes_ok() {
    ...
    for &(evalue, data) in tests {
        let value = parse_der(data, |reader| reader.read_bytes()).unwrap();
        assert_eq!(value, evalue);
    }
}
...
fn test_der_read_bytes_err() {
    ...
    for &data in tests {
        parse_der(data, |reader| reader.read_bytes()).unwrap_err();
    }
}
...
fn test_ber_read_bytes_ok() {
    ...
    for &(evalue, data) in tests {
        let value = parse_ber(data, |reader| reader.read_bytes()).unwrap();
        assert_eq!(value, evalue);
    }
}
...
fn test_ber_read_bytes_err() {
    ...
    for &data in tests {
        parse_ber(data, |reader| reader.read_bytes()).unwrap_err();
    }
}

#[test]
fn test_der_read_null_ok() {
    let value = parse_der(&[5, 0], |reader| reader.read_null()).unwrap();
    assert_eq!(value, ());
}
...
fn test_der_read_null_err() {
    ...
    for &data in tests {
        parse_der(data, |reader| reader.read_null()).unwrap_err();
    }
}

#[test]
fn test_ber_read_null_ok() {
    let value = parse_ber(&[5, 0], |reader| reader.read_null()).unwrap();
    assert_eq!(value, ());
}
...
fn test_ber_read_null_err() {
    ...
    for &data in tests {
        parse_ber(data, |reader| reader.read_null()).unwrap_err();
    }
}
...
fn test_der_read_sequence_ok() {
    ...
    let tests: &[((), &[u8])] = &[((), &[48, 0])];
    for &(evalue, data) in tests {
        let value = parse_der(data, |reader| reader.read_sequence(|_| Ok(()))).unwrap();
        assert_eq!(value, evalue);
    }
}
...
fn test_der_read_tagged_der_err() {
    ...
    for &data in tests {
        parse_der(data, |reader| reader.read_tagged_der()).unwrap_err();
    }
}

#[test]
fn test_ber_read_sequence_ok() {
    let tests: &[((i64, bool), TaggedDerValue, &[u8])] = &[
        // Definite-length encoding
        (
            (10, true),
            TaggedDerValue::from_tag_and_bytes(TAG_SEQUENCE, [2, 1, 10, 1, 1, 255].to_vec()),
            &[48, 6, 2, 1, 10, 1, 1, 255],
        ),
        // Definite-length encoding
        (
            (266, true),
            TaggedDerValue::from_tag_and_bytes(TAG_SEQUENCE, [2, 2, 1, 10, 1, 1, 255].to_vec()),
            &[48, 7, 2, 2, 1, 10, 1, 1, 255],
        ),
        // Indefinite-length encoding
        (
            (10, true),
            TaggedDerValue::from_tag_and_bytes(TAG_SEQUENCE, [2, 1, 10, 1, 1, 255, 0, 0].to_vec()),
            &[48, 128, 2, 1, 10, 1, 1, 255, 0, 0],
        ),
        // Indefinite-length encoding
        (
    ...
    for &(evalue, ref ervalue, data) in tests {
        ...
        .unwrap();
        assert_eq!(value, evalue);
        let rvalue = parse_ber(data, |reader| reader.read_tagged_der()).unwrap();
        assert_eq!(&rvalue, ervalue);
    }

    let tests: &[((), &[u8])] = &[((), &[48, 0]), ((), &[48, 128, 0, 0])];
    for &(evalue, data) in tests {
        let value = parse_ber(data, |reader| reader.read_sequence(|_| Ok(()))).unwrap();
        assert_eq!(value, evalue);
    }
}
...
fn test_der_read_set_ok() {
    let tests: &[((i64, Vec<u8>, i64, Vec<u8>), &[u8])] = &[(
        ...
        &[
            49, 32, 187, 5, 2, 3, 6, 248, 86, 156, 3, 6, 248, 85, 191, 149, 140, 77, 5, 4, 3, 66,
            97, 114, 191, 149, 140, 78, 5, 4, 3, 70, 111, 111,
        ],
    )];
    for &(ref evalue, data) in tests {
        let value = parse_der(data, |reader| {
            reader.read_set(|reader| {
                let a = try!(try!(reader.next(&[Tag::context(28)]))
                    .read_tagged_implicit(Tag::context(28), |reader| { reader.read_i64() }));
                let b = try!(try!(reader.next(&[Tag::context(345678)]))
                    .read_tagged(Tag::context(345678), |reader| { reader.read_bytes() }));
                let c = try!(try!(reader.next(&[Tag::context(27)]))
                    .read_tagged(Tag::context(27), |reader| { reader.read_i64() }));
                let d = try!(try!(reader.next(&[Tag::context(345677)]))
                    .read_tagged(Tag::context(345677), |reader| { reader.read_bytes() }));
                return Ok((a, b, c, d));
            })
        })
        .unwrap();
...
fn test_der_read_set_err() {
    let tests: &[&[u8]] = &[
        ...
        &[0, 1, 0],
        &[
            17, 32, 187, 5, 2, 3, 6, 248, 86, 156, 3, 6, 248, 85, 191, 149, 140, 77, 5, 4, 3, 66,
            97, 114, 191, 149, 140, 78, 5, 4, 3, 70, 111, 111,
        ],
        &[
            113, 32, 187, 5, 2, 3, 6, 248, 86, 156, 3, 6, 248, 85, 191, 149, 140, 77, 5, 4, 3, 66,
            97, 114, 191, 149, 140, 78, 5, 4, 3, 70, 111, 111,
        ],
        &[
            48, 32, 187, 5, 2, 3, 6, 248, 86, 156, 3, 6, 248, 85, 191, 149, 140, 77, 5, 4, 3, 66,
            97, 114, 191, 149, 140, 78, 5, 4, 3, 70, 111, 111,
        ],
        &[
            49, 33, 187, 5, 2, 3, 6, 248, 86, 156, 3, 6, 248, 85, 191, 149, 140, 77, 5, 4, 3, 66,
            97, 114, 191, 149, 140, 78, 5, 4, 3, 70, 111, 111,
        ],
        &[
            49, 33, 187, 5, 2, 3, 6, 248, 86, 156, 3, 6, 248, 85, 191, 149, 140, 77, 5, 4, 3, 66,
            97, 114, 191, 149, 140, 78, 5, 4, 3, 70, 111, 111, 0,
        ],
        &[
            49, 31, 187, 5, 2, 3, 6, 248, 86, 156, 3, 6, 248, 85, 191, 149, 140, 77, 5, 4, 3, 66,
            97, 114, 191, 149, 140, 78, 5, 4, 3, 70, 111, 111,
        ],
        &[
            49, 31, 187, 5, 2, 3, 6, 248, 86, 156, 3, 6, 248, 85, 191, 149, 140, 77, 5, 4, 3, 66,
            97, 114, 191, 149, 140, 78, 5, 4, 3, 70, 111,
        ],
        &[
            49, 22, 187, 5, 2, 3, 6, 248, 86, 156, 3, 6, 248, 85, 191, 149, 140, 77, 5, 4, 3, 66,
            97, 114,
        ],
        &[
            49, 32, 187, 5, 2, 3, 6, 248, 86, 156, 3, 6, 248, 85, 191, 149, 140, 77, 5, 4, 3, 66,
            97, 114, 191, 149, 140, 78, 5, 4, 3, 70, 111, 111, 191, 149, 140, 79, 5, 4, 3, 70, 111,
            111,
        ],
        &[
            49, 128, 187, 5, 2, 3, 6, 248, 86, 156, 3, 6, 248, 85, 191, 149, 140, 77, 5, 4, 3, 66,
            97, 114, 191, 149, 140, 78, 5, 4, 3, 70, 111, 111, 0, 0,
        ],
        &[
            49, 32, 156, 3, 6, 248, 85, 187, 5, 2, 3, 6, 248, 86, 191, 149, 140, 77, 5, 4, 3, 66,
            97, 114, 191, 149, 140, 78, 5, 4, 3, 70, 111, 111,
        ],
        &[
            49, 32, 187, 5, 2, 3, 6, 248, 86, 191, 149, 140, 77, 5, 4, 3, 66, 97, 114, 156, 3, 6,
            248, 85, 191, 149, 140, 78, 5, 4, 3, 70, 111, 111,
        ],
        &[
            49, 32, 187, 5, 2, 3, 6, 248, 86, 156, 3, 6, 248, 85, 191, 149, 140, 78, 5, 4, 3, 70,
            111, 111, 191, 149, 140, 77, 5, 4, 3, 66, 97, 114,
        ],
    ];
    for &data in tests {
        parse_der(data, |reader| {
...
fn test_ber_read_set_ok() {
    let tests: &[((i64, Vec<u8>, i64, Vec<u8>), &[u8])] = &[
        (
            (456789, b"Foo".to_vec(), 456790, b"Bar".to_vec()),
            &[
                49, 32, 187, 5, 2, 3, 6, 248, 86, 156, 3, 6, 248, 85, 191, 149, 140, 77, 5, 4, 3,
                66, 97, 114, 191, 149, 140, 78, 5, 4, 3, 70, 111, 111,
            ],
        ),
        (
            (456789, b"Foo".to_vec(), 456790, b"Bar".to_vec()),
            &[
                49, 128, 187, 5, 2, 3, 6, 248, 86, 156, 3, 6, 248, 85, 191, 149, 140, 77, 5, 4, 3,
                66, 97, 114, 191, 149, 140, 78, 5, 4, 3, 70, 111, 111, 0, 0,
            ],
        ),
        (
            (456789, b"Foo".to_vec(), 456790, b"Bar".to_vec()),
            &[
                49, 32, 156, 3, 6, 248, 85, 187, 5, 2, 3, 6, 248, 86, 191, 149, 140, 77, 5, 4, 3,
                66, 97, 114, 191, 149, 140, 78, 5, 4, 3, 70, 111, 111,
            ],
        ),
        (
            (456789, b"Foo".to_vec(), 456790, b"Bar".to_vec()),
            &[
                49, 32, 187, 5, 2, 3, 6, 248, 86, 191, 149, 140, 77, 5, 4, 3, 66, 97, 114, 156, 3,
                6, 248, 85, 191, 149, 140, 78, 5, 4, 3, 70, 111, 111,
            ],
        ),
        (
            (456789, b"Foo".to_vec(), 456790, b"Bar".to_vec()),
            &[
                49, 32, 187, 5, 2, 3, 6, 248, 86, 156, 3, 6, 248, 85, 191, 149, 140, 78, 5, 4, 3,
                70, 111, 111, 191, 149, 140, 77, 5, 4, 3, 66, 97, 114,
            ],
        ),
    ];
    for &(ref evalue, data) in tests {
...
fn test_der_read_set_of_ok() {
    ...
    let tests: &[(&[i64], &[u8])] = &[
        ...
        (
            ...
            &[
                49, 19, 2, 1, 127, 2, 1, 128, 2, 2, 0, 128, 2, 2, 255, 127, 2, 3, 0, 128, 0,
            ],
        ),
    ];
    for &(evalue, data) in tests {
        let value = parse_der(data, |reader| {
            reader.collect_set_of(|reader| reader.read_i64())
        })
        .unwrap();
...
fn test_der_read_tagged_ok() {
    let tests: &[(i64, &[u8])] = &[(10, &[163, 3, 2, 1, 10]), (266, &[163, 4, 2, 2, 1, 10])];
    for &(evalue, data) in tests {
        let value = parse_der(data, |reader| {
            reader.read_tagged(Tag::context(3), |reader| reader.read_i64())
        })
        .unwrap();
        assert_eq!(value, evalue);
    }
...
fn test_der_read_tagged_implicit_ok() {
    let tests: &[(i64, &[u8])] = &[(10, &[131, 1, 10]), (266, &[131, 2, 1, 10])];
    for &(evalue, data) in tests {
        let value = parse_der(data, |reader| {
            reader.read_tagged_implicit(Tag::context(3), |reader| reader.read_i64())
        })
        .unwrap();
        assert_eq!(value, evalue);
    }
